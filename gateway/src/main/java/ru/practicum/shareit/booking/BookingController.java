...
public class BookingController {
    ...
    @GetMapping("/owner")
    public ResponseEntity<Object> getBookingsForOwner(@RequestParam(defaultValue = "ALL") String stateParam,
                                                      @RequestHeader("X-Sharer-User-Id") Long userId) {
        BookingState state = BookingState.from(stateParam)
                .orElseThrow(() -> new IllegalArgumentException("Unknown state: " + stateParam));
        log.info("Get booking for owner - %d".formatted(userId));
    ...
    public ResponseEntity<Object> change_approved(@PathVariable Long bookingId,
        ...
        log.info("Change status approved");
        return bookingClient.change_approved(bookingId, userId, approved);
    }
}
